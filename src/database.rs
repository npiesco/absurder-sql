use crate::types::{ColumnValue, DatabaseConfig, DatabaseError, QueryResult, Row};
...
impl<'conn> PreparedStatement<'conn> {
    ...
    pub async fn execute(&mut self, params: &[ColumnValue]) -> Result<QueryResult, DatabaseError> {
        ...
        log::debug!(
            "Prepared statement executed in {:.2}ms, {} rows returned",
...
impl SqliteIndexedDB {
    pub async fn new(config: DatabaseConfig) -> Result<Self, DatabaseError> {
        ...
        #[cfg(feature = "fs_persist")]
        {
            ...
            let storage = BlockStorage::new(&storage_name)
                .await
            ...
            if let Some(parent) = db_file_path.parent() {
                std::fs::create_dir_all(parent).map_err(|e| {
                    DatabaseError::new("IO_ERROR", &format!("Failed to create directory: {}", e))
                })?;
            }
        ...
        {
            let connection = Connection::open_in_memory().map_err(|e| DatabaseError::from(e))?;
    ...
    fn apply_pragmas(&mut self) -> Result<(), DatabaseError> {
        ...
        if let Some(cache_size) = self.config.cache_size {
            ...
            let _ = stmt.query_map([], |_| Ok(())).map_err(|e| {
    ...
    pub async fn execute_with_params(
        ...
        if is_select {
            ...
            for row in rows {
                result
                    .rows
                    .push(row.map_err(|e| DatabaseError::from(e).with_sql(sql))?);
            }
        } else {
        ...
        log::debug!(
            "SQL executed in {:.2}ms, {} rows affected/returned",
            result.execution_time_ms,
            if is_select {
                result.rows.len()
            } else {
    ...
    pub async fn execute_batch(&mut self, statements: &[String]) -> Result<(), DatabaseError> {
        ...
        log::debug!(
            "Batch of {} statements executed in {:.2}ms",
            statements.len(),
            duration
        );
    ...
    pub async fn new_encrypted(config: DatabaseConfig, key: &str) -> Result<Self, DatabaseError> {
        log::info!(
            "Creating encrypted SQLiteIndexedDB with config: {:?}",
            config
        );
        ...
        #[cfg(feature = "fs_persist")]
        {
            ...
            if db_file_path.exists() && db_file_path.is_dir() {
                std::fs::remove_dir_all(&db_file_path).map_err(|e| {
                    DatabaseError::new(
                        "IO_ERROR",
                        &format!("Failed to remove existing VFS directory: {}", e),
                    )
                })?;
            }
            ...
            connection
                .execute_batch(&format!("PRAGMA key = '{}';", escaped_key))
                .map_err(|e| {
                    DatabaseError::new(
                        "ENCRYPTION_ERROR",
            ...
            connection
                ...
                .map_err(|e| {
                    DatabaseError::new(
                        "ENCRYPTION_ERROR",
                        &format!("Failed to verify encryption: {}", e),
                    )
                })?;

            // Drop the test table
            connection
                .execute("DROP TABLE _encryption_check", [])
                .map_err(|e| {
                    DatabaseError::new(
                        "ENCRYPTION_ERROR",
                        &format!("Failed to cleanup test table: {}", e),
                    )
                })?;

            log::info!(
                "Encrypted native database opened with filesystem persistence: {:?}",
                db_file_path
            );
    ...
    pub async fn rekey(&self, new_key: &str) -> Result<(), DatabaseError> {
        ...
        self.connection
            .execute_batch(&format!("PRAGMA rekey = '{}';", escaped_key))
            .map_err(|e| {
                DatabaseError::new(
                    "ENCRYPTION_ERROR",
                    &format!("Failed to rekey database: {}", e),
                )
            })?;

        // Verify new key works by executing a test pragma
        self.connection
            .execute_batch("PRAGMA cipher_version;")
            .map_err(|e| {
                DatabaseError::new(
                    "ENCRYPTION_ERROR",
                    &format!("New key verification failed: {}", e),
                )
            })?;
