...
async fn test_heartbeat_stops_after_close() {
    ...
    let initial_heartbeat = initial_heartbeat
        .expect("Heartbeat should exist immediately after becoming leader");
    ...
    assert!(!updated_heartbeat.is_empty(), "Heartbeat should be updating");
    ...
    let final_heartbeat = storage
        .get_item(&heartbeat_key)
        .unwrap();
    
    assert!(final_heartbeat.is_none() || final_heartbeat == Some("".to_string()), 
