...
async fn test_current_checksum_behavior_write_read() {
    ...
    storage.write_block(42, test_data.clone()).await.expect("Should write block");
    ...
    assert_eq!(read_data, test_data, "Data should be preserved and checksum validated");
}
...
async fn test_current_checksum_behavior_multiple_blocks() {
    ...
    for i in 1..=3 {
        ...
        let read_data = storage.read_block(i).await.expect(&format!("Should read block {}", i));
        assert_eq!(read_data, expected_data, "Block {} should preserve data and validate checksum", i);
    }
}
...
async fn test_current_checksum_behavior_data_corruption_detection() {
    ...
    // Write and sync
    storage.write_block(100, original_data.clone()).await.expect("Should write block");
    storage.sync().await.expect("Should sync");

    // Normal read should work
    let read_data = storage.read_block(100).await.expect("Should read uncorrupted data");
    assert_eq!(read_data, original_data, "Uncorrupted data should read correctly");
...
async fn test_current_checksum_behavior_overwrite_block() {
    ...
    storage.write_block(50, data1.clone()).await.expect("Should write first data");
    storage.sync().await.expect("Should sync");

    // Verify first data
    let read1 = storage.read_block(50).await.expect("Should read first data");
    ...
    storage.write_block(50, data2.clone()).await.expect("Should overwrite block");
    storage.sync().await.expect("Should sync");

    // Verify second data (checksum should have been updated)
    let read2 = storage.read_block(50).await.expect("Should read overwritten data");
    assert_eq!(read2, data2, "Overwrite should be preserved with updated checksum");
}
...
async fn test_current_checksum_behavior_across_instances() {
    ...
    // First instance - write data
    {
        ...
        storage1.write_block(25, data).await.expect("Should write in first instance");
    ...
    // Second instance - read same data (checksum should be preserved and validated)
    {
        ...
        assert_eq!(read_data, expected_data, "Data should persist with valid checksum across instances");
    }
}