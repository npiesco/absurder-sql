...
use absurder_sql::*;
use absurder_sql::{QueryResult, ColumnValue};
...
async fn test_import_close_reopen_persistence() {
    ...
    Database::delete_database(format!("{}.db", source_db_name)).await.ok();
    Database::delete_database(format!("{}.db", target_db_name)).await.ok();
    ...
    if let Some(window) = window() {
        if let Ok(Some(storage)) = window.local_storage() {
            for db in &[&source_db_name, &target_db_name] {
                ...
                storage.remove_item(&format!("datasync_leader_{}", db_key)).ok();
                storage.remove_item(&format!("datasync_instances_{}", db_key)).ok();
                storage.remove_item(&format!("datasync_heartbeat_{}", db_key)).ok();
            }
        }
    }
    ...
    let mut source_db = Database::new(config1).await
        .expect("Should create source database");
    
    source_db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)").await
        .expect("Should create table");
    source_db.execute("INSERT INTO test (id, name) VALUES (1, 'Alice'), (2, 'Bob')").await
        .expect("Should insert data");
    
    // Verify source data
    let result_js = source_db.execute("SELECT COUNT(*) as count FROM test").await
        .expect("Should query source");
    let result: QueryResult = serde_wasm_bindgen::from_value(result_js)
    ...
    let page_count_result = source_db.execute("PRAGMA page_count").await.expect("Should get page count");
    web_sys::console::log_1(&format!("Source page_count: {:?}", page_count_result).into());
    let page_size_result = source_db.execute("PRAGMA page_size").await.expect("Should get page size");
    ...
    source_db.sync().await.expect("Should sync source before export");
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        with_global_storage(|storage_map| {
            if let Some(db_storage) = storage_map.borrow().get(&format!("{}.db", source_db_name)) {
                web_sys::console::log_1(&format!("DEBUG: Source GLOBAL_STORAGE has {} blocks before export", db_storage.len()).into());
                web_sys::console::log_1(&format!("DEBUG: Source block IDs: {:?}", db_storage.keys().collect::<Vec<_>>()).into());
            }
        });
    }
    ...
    let export_bytes = source_db.export_to_file().await
    ...
    let mut target_db = Database::new(config2.clone()).await
        .expect("Should create target database");
    
    // Import the data (NOTE: this closes target_db)
    target_db.import_from_file(export_bytes).await
    ...
    let mut target_db = Database::new(config2).await
        .expect("Should reopen target database");
    
    // Check database integrity
    web_sys::console::log_1(&"Running integrity check on imported database".into());
    let integrity_result = target_db.execute("PRAGMA integrity_check").await
    ...
    let mut reopened_db = Database::new(config3).await
        .expect("Should reopen target database");
    
    // Query the data
    let result_js = reopened_db.execute("SELECT * FROM test ORDER BY id").await
    ...
    web_sys::console::log_1(&"[PASS] Data persisted correctly through import -> close -> reopen".into());
}
...
async fn test_concurrent_import_close_reopen() {
    ...
    if let Some(window) = window() {
        if let Ok(Some(storage)) = window.local_storage() {
            ...
            for i in 0..4 {
                ...
                storage.remove_item(&format!("datasync_leader_{}", db_name)).ok();
                storage.remove_item(&format!("datasync_instances_{}", db_name)).ok();
                storage.remove_item(&format!("datasync_heartbeat_{}", db_name)).ok();
    ...
    let mut source_db = Database::new(config_source).await
        .expect("Should create source database");
    
    source_db.execute("CREATE TABLE test (id INTEGER, data TEXT)").await
        .expect("Should create table");
    source_db.execute("INSERT INTO test VALUES (100, 'test data')").await
    ...
    for i in 0..3 {
        ...
        let fut = async move {
            ...
            db.import_from_file(bytes_clone).await
            ...
            let mut reopened = Database::new(config2).await
                .expect("Should reopen");
            
            let result_js = reopened.execute("SELECT * FROM test").await
                .expect("Should query");
            let result: QueryResult = serde_wasm_bindgen::from_value(result_js)
                .expect("Should deserialize");
            ...
            web_sys::console::log_1(&format!("[DB {}] Verified {} rows", i, result.rows.len()).into());
