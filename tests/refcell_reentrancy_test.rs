...
mod refcell_reentrancy_tests {
    ...
    impl MockStorage {
        ...
        fn prepare_statement(&mut self, storage_rc: &Rc<RefCell<MockStorage>>) -> Result<(), String> {
            ...
            match storage_rc.try_borrow_mut() {
                ...
                Err(_) => {
                    Err("RefCell reentrancy panic would occur here".to_string())
                }
            }
        }
    }
    ...
    fn test_refcell_reentrancy_detected_with_try_borrow() {
        ...
        assert_eq!(result.unwrap_err(), "RefCell reentrancy panic would occur here");
    }
...
mod actual_wasm_reentrancy {
    ...
    async fn test_real_blockstorage_reentrancy_panic() {
        ...
        let result = db2.execute("CREATE TABLE test2 (id INTEGER PRIMARY KEY, value TEXT)").await;
        ...
        assert!(result.is_ok(), "DDL should work without reentrancy panic: {:?}", result.err());
        
        // Cleanup
        db1.close().await.ok();
        db2.close().await.ok();
        Database::delete_database(format!("{}.db", db_name)).await.ok();
    }
    ...
    async fn test_concurrent_operations_multiple_instances() {
        ...
        assert!(results.0.is_ok(), "db1 insert failed: {:?}", results.0.err());
        assert!(results.1.is_ok(), "db2 insert failed: {:?}", results.1.err());
        assert!(results.2.is_ok(), "db3 insert failed: {:?}", results.2.err());
