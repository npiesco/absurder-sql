...
async fn test_enable_optimistic_mode() {
    ...
    // Create database
    let mut db = Database::new_wasm("optimistic_mode_test".to_string()).await.unwrap();
...
async fn test_track_pending_writes() {
    ...
    let mut db = Database::new_wasm("optimistic_track_test".to_string()).await.unwrap();
    
    // Enable optimistic mode
    db.enable_optimistic_updates(true).await.unwrap();
    
    // Setup table
    db.execute("CREATE TABLE IF NOT EXISTS items (id INT PRIMARY KEY, name TEXT)").await.unwrap();
    ...
    db.track_optimistic_write("INSERT INTO items VALUES (1, 'pending')".to_string()).await.unwrap();
...
async fn test_clear_pending_writes() {
    ...
    let mut db = Database::new_wasm("optimistic_clear_test".to_string()).await.unwrap();
    
    // Enable optimistic mode
    db.enable_optimistic_updates(true).await.unwrap();
    
    // Track some writes
    db.track_optimistic_write("INSERT INTO items VALUES (1, 'item1')".to_string()).await.unwrap();
    db.track_optimistic_write("INSERT INTO items VALUES (2, 'item2')".to_string()).await.unwrap();
    ...
    assert_eq!(pending_count_after, 0, "Should have 0 pending writes after clear");
