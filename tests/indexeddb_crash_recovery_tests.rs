...
async fn test_indexeddb_recovery_finalize_incomplete_transaction() {
    ...
    storage1
        .sync()
        .await
        .expect("sync - this should complete normally for now");
    ...
    assert_eq!(
        recovered_marker, post_sync_marker,
        "Recovery should preserve committed state"
    );

    // Data should be visible after recovery
    let recovered_data = storage2
        .read_block_sync(block_id)
        .expect("read recovered data");
...
async fn test_indexeddb_recovery_rollback_incomplete_transaction() {
    ...
    storage1
        .write_block(block2, data2.clone())
        .await
        .expect("write block2");
    ...
    assert_eq!(
        pre_crash_marker, baseline_marker,
        "Commit marker should not advance before sync"
    );
    ...
    web_sys::console::log_1(
        &format!("Recovered marker after rollback: {}", recovered_marker).into(),
    );
    ...
    assert_eq!(
        recovered_marker, baseline_marker,
        "Recovery should rollback to last committed state"
    );

    // Block1 should be visible (committed)
    let recovered_data1 = storage2
        .read_block_sync(block1)
    ...
    assert_eq!(
        recovered_data2, data2,
        "Block2 should be visible (current implementation persists immediately)"
    );
}
...
async fn test_indexeddb_recovery_commit_marker_monotonic() {
    ...
    for i in 0..3 {
        ...
        assert!(
            ...
            "Commit marker should be monotonic across recovery, cycle {}",
            i
        );
...
async fn test_indexeddb_recovery_multiple_databases() {
    ...
    assert_eq!(
        recovered1.get_commit_marker(),
        marker1,
        "DB1 should recover independently"
    );
    assert_eq!(
        recovered2.get_commit_marker(),
        marker2,
        "DB2 should recover independently"
    );

    let recovered_data1 = recovered1
        .read_block_sync(block1)
        .expect("read recovered data1");
    let recovered_data2 = recovered2
        .read_block_sync(block2)
        .expect("read recovered data2");
...
async fn test_indexeddb_rollback_deletes_blocks() {
    ...
    assert!(
        block2_exists_before,
        "Block2 should exist in storage before rollback"
    );
    ...
    web_sys::console::log_1(
        &format!("Block2 exists after finalize: {}", block2_exists_after).into(),
    );
    ...
    assert_eq!(
        final_marker, 2,
        "Commit marker should advance to 2 after finalize"
    );
...
async fn test_indexeddb_rollback_deletes_orphaned_blocks() {
    ...
    web_sys::console::log_1(
        &format!(
            "Recovery action for inconsistent versions: {:?}",
            recovery_action
        )
        .into(),
    );
    ...
    web_sys::console::log_1(
        &format!(
            "Block2 in cache: {}, Block3 in cache: {}",
            block2_in_cache, block3_in_cache
        )
        .into(),
    );
    ...
    let read2_result = storage2
        .read_block_sync(block2)
        .expect("read should succeed with zeros");
    let read3_result = storage2
        .read_block_sync(block3)
        .expect("read should succeed with zeros");

    assert_eq!(
        read2_result,
        vec![0u8; 4096],
        "Block2 should return zeros after rollback"
    );
    assert_eq!(
        read3_result,
        vec![0u8; 4096],
        "Block3 should return zeros after rollback"
    );

    web_sys::console::log_1(
        &"Test completed - rollback successfully deleted orphaned blocks from IndexedDB".into(),
    );
}
