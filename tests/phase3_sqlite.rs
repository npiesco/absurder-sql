...
async fn test_create_table() {
    ...
    let result = db.execute(create_sql).await.expect("Should create table");
...
async fn test_insert_data() {
    ...
    let result = db.execute(insert_sql).await.expect("Should insert data");
...
async fn test_select_data() {
    ...
    db.execute("INSERT INTO products (name, price) VALUES ('Gadget', 29.99)")
        .await
        .expect("Should insert data");
...
async fn test_update_data() {
    ...
    db.execute("INSERT INTO items (name, quantity) VALUES ('Item1', 10)")
        .await
        .expect("Should insert data");

    db.execute("INSERT INTO items (name, quantity) VALUES ('Item2', 20)")
        .await
        .expect("Should insert data");

    // Update
    let result = db
        .execute("UPDATE items SET quantity = 15 WHERE name = 'Item1'")
        .await
    ...
    let select_result = db
        .execute("SELECT quantity FROM items WHERE name = 'Item1'")
        .await
...
async fn test_delete_data() {
    ...
    // Setup
    db.execute("CREATE TABLE temp_data (id INTEGER PRIMARY KEY, value TEXT)")
        .await
        .expect("Should create table");

    db.execute("INSERT INTO temp_data (value) VALUES ('keep')")
        .await
        .expect("Should insert data");

    db.execute("INSERT INTO temp_data (value) VALUES ('delete')")
        .await
        .expect("Should insert data");

    // Delete
    let result = db
        .execute("DELETE FROM temp_data WHERE value = 'delete'")
        .await
...
async fn test_parametrized_queries() {
    ...
    // Setup
    db.execute("CREATE TABLE param_test (id INTEGER PRIMARY KEY, name TEXT, score REAL)")
        .await
        .expect("Should create table");
    ...
    let result = db
        .execute_with_params(
            "INSERT INTO param_test (name, score) VALUES (?, ?)",
            &params,
        )
        .await
    ...
    let select_result = db
        .execute_with_params(
            "SELECT name, score FROM param_test WHERE name = ?",
            &select_params,
        )
        .await
...
async fn test_multiple_data_types() {
    ...
    let result = db
        .execute("SELECT int_val, real_val, text_val, blob_val, null_val FROM type_test")
        .await
...
async fn test_database_close() {
    ...
    db.execute("CREATE TABLE close_test (id INTEGER)")
        .await
        .expect("Should create table");

    db.execute("INSERT INTO close_test (id) VALUES (1)")
        .await
        .expect("Should insert data");
