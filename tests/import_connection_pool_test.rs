...
async fn test_import_connection_pool_isolation() {
    ...
    let mut source_db = Database::new(DatabaseConfig {
        ...
        ..Default::default()
    }).await.expect("Should create source");
    
    source_db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT)").await.expect("Should create table");
    source_db.execute("INSERT INTO test VALUES (1, 'data')").await.expect("Should insert");
    ...
    let mut target_db = Database::new(DatabaseConfig {
        ...
        ..Default::default()
    }).await.expect("Should create target");
    ...
    web_sys::console::log_1(&format!("Connection exists in pool before import: {}", exists_before).into());
    
    // Step 3: Import (calls close() which might just decrement ref_count)
    target_db.import_from_file(export_bytes).await.expect("Should import");
    ...
    web_sys::console::log_1(&format!("Connection exists in pool after import: {}", exists_after_import).into());
    
    if exists_after_import {
        web_sys::console::log_1(&"BUG CONFIRMED: Connection pool still has entry after import!".into());
        web_sys::console::log_1(&"This means new Database instances will reuse stale SQLite connection".into());
    }
    ...
    let mut target_db_new = Database::new(DatabaseConfig {
        name: target_name.clone(),
        ..Default::default()
    }).await.expect("Should create after import");
    
    // Try to read imported data
    let result = target_db_new.execute("SELECT * FROM test WHERE id = 1").await;
