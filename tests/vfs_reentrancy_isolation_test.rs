...
async fn test_vfs_reentrancy_during_read() {
    ...
    assert!(result.is_ok(), "Query should succeed without reentrancy panic: {:?}", result);
    ...
    // Cleanup
    db.close().await.ok();
    Database::delete_database("vfs_reentrancy_test".to_string()).await.ok();
}
...
async fn test_schema_operation_reentrancy() {
    ...
    let result = db.execute("CREATE TABLE posts (id INTEGER PRIMARY KEY, title TEXT)")
        .await;
    
    assert!(result.is_ok(), "Second table creation should succeed: {:?}", result);
    
    // Cleanup
    db.close().await.ok();
    Database::delete_database("schema_reentrancy_test".to_string()).await.ok();
}
...
async fn test_write_triggers_read_reentrancy() {
    ...
    // RED: Should panic
    // GREEN: Interior mutability allows nested access
    let result = db.execute("UPDATE data SET value = 'updated' WHERE id = 1")
        .await;
    
    assert!(result.is_ok(), "Update should succeed without reentrancy panic: {:?}", result);
    
    // Verify update worked
    let rows = db.query("SELECT value FROM data WHERE id = 1")
        .await
        .expect("query");
    ...
    // Cleanup
    db.close().await.ok();
    Database::delete_database("write_read_reentrancy".to_string()).await.ok();
}
