...
async fn test_forced_leader_maintains_lease() {
    ...
    storage2.start_leader_election().await.expect("force leadership");
    sleep_ms(300).await;
    
    // Verify follower is now leader
    assert!(storage2.is_leader().await, "Second instance should be leader after forcing");
    web_sys::console::log_1(&"Second instance forced leadership successfully".into());
    
    // Wait 6 seconds (longer than 5 second lease)
    web_sys::console::log_1(&"Waiting 6 seconds to test if lease is maintained via heartbeat...".into());
    sleep_ms(6000).await;
