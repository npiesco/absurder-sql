...
async fn test_delete_all_blocks_after_close() {
    ...
    let mut db = Database::new(config.clone()).await.expect("Should create database");
    db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, data TEXT)").await.expect("Should create table");
    db.execute("INSERT INTO test VALUES (1, 'row1')").await.expect("Should insert row 1");
    db.execute("INSERT INTO test VALUES (2, 'row2')").await.expect("Should insert row 2");
    db.execute("INSERT INTO test VALUES (3, 'row3')").await.expect("Should insert row 3");

    // Force sync to IndexedDB
    db.sync().await.expect("Should sync to IndexedDB");

    // Verify data persisted
    let count_result = db.execute("SELECT COUNT(*) FROM test").await.expect("Should count");
    ...
    web_sys::console::log_1(&format!("Step 3: Clearing all storage for db_name='{}' ...", db_name).into());

    // Clear memory storage (GLOBAL_STORAGE, GLOBAL_METADATA, registry, connection pool)
    absurder_sql::storage::import::clear_database_storage(&db_name).await
    ...
    delete_all_database_blocks_from_indexeddb(&db_name).await
    ...
    let mut new_db = Database::new(config).await.expect("Should create new database");

    // The table should not exist because IndexedDB was cleared
    // Check what tables exist
    let table_check = new_db.execute("SELECT name FROM sqlite_master WHERE type='table'").await;
    web_sys::console::log_1(&format!("[DEBUG] sqlite_master query result: {:?}", table_check).into());
    ...
    match table_check {
        Ok(result) => {
            ...
            if result_str.contains("\"test\"") {
                panic!("FAILED: Table 'test' still exists - IndexedDB was NOT cleared properly! Result: {}", result_str);
            } else {
                web_sys::console::log_1(&"SUCCESS: Table 'test' does not exist (storage was cleared)".into());
            }
        }
        Err(e) => {
            // Error might indicate database was cleared and SQLite can't read it
            web_sys::console::log_1(&format!("[DEBUG] Query error (might be expected): {:?}", e).into());
            web_sys::console::log_1(&"SUCCESS: Query failed - database appears to be cleared".into());
        }
    }
...
async fn test_delete_all_enables_clean_import() {
    ...
    let mut source_db = Database::new(source_config).await.expect("Should create source");
    source_db.execute("CREATE TABLE source_data (id INTEGER, value TEXT)").await.expect("Create table");
    source_db.execute("INSERT INTO source_data VALUES (100, 'imported_value')").await.expect("Insert");
    ...
    let mut target_db = Database::new(target_config.clone()).await.expect("Should create target");
    target_db.execute("CREATE TABLE original_data (x INTEGER)").await.expect("Create original table");
    target_db.execute("INSERT INTO original_data VALUES (999)").await.expect("Insert original");
    ...
    absurder_sql::storage::import::clear_database_storage(&target_name).await
        .expect("Should clear target memory storage");
    delete_all_database_blocks_from_indexeddb(&target_name).await
    ...
    absurder_sql::storage::import::import_database_from_bytes(
        &target_name,
        export_bytes.to_vec()
    ).await.expect("Should import");
    ...
    let source_check = reopened.execute("SELECT value FROM source_data WHERE id = 100").await;
    match source_check {
        Ok(result) => {
            let result_str = format!("{:?}", result);
            if result_str.contains("imported_value") {
                ...
                panic!("FAILED: source_data table exists but wrong data: {}", result_str);
            }
        }
