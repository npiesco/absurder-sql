...
async fn test_simultaneous_database_creation() {
    ...
    let mut db1 = absurder_sql::Database::new(config.clone()).await.expect("Tab 1 should create database");
    log::info!("TEST: DB1 created successfully");
    
    log::info!("TEST: Opening DB2");
    let mut db2 = absurder_sql::Database::new(config.clone()).await.expect("Tab 2 should reuse database");
    ...
    db1.execute("CREATE TABLE tab1_data (id INTEGER, value TEXT)").await.expect("Tab 1 create table");
    
    log::info!("TEST: DB1 inserting data");
    db1.execute("INSERT INTO tab1_data VALUES (1, 'from_tab1')").await.expect("Tab 1 insert");
    
    log::info!("TEST: DB2 dropping old table");
    db2.execute("DROP TABLE IF EXISTS tab2_data").await.ok();
    
    log::info!("TEST: DB2 creating table");
    db2.execute("CREATE TABLE tab2_data (id INTEGER, value TEXT)").await.expect("Tab 2 create table");
    
    log::info!("TEST: DB2 inserting data");
    db2.execute("INSERT INTO tab2_data VALUES (2, 'from_tab2')").await.expect("Tab 2 insert");
