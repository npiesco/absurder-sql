...
async fn test_import_invalidates_cache() {
    ...
    source_db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT)").await.expect("Should create table");
    source_db.execute("INSERT INTO test VALUES (1, 'test_data')").await.expect("Should insert");
    ...
    let result_before = target_db.execute("PRAGMA page_count").await.expect("Should get page count");
    web_sys::console::log_1(&format!("BEFORE IMPORT page_count: {:?}", result_before).into());
    
    // Step 5: Import (this will close the connection)
    target_db.import_from_file(export_bytes.clone()).await.expect("Should import");
    ...
    let mut reopened_db = Database::new(target_config.clone()).await.expect("Should create new instance");
    ...
    match integrity_result {
        Ok(_) => {
            web_sys::console::log_1(&"SUCCESS: Data readable after import (cache was invalidated)".into());
        }
        Err(e) => {
...
async fn test_import_with_close_first() {
    ...
    target_db = Database::new(target_config.clone()).await.expect("Should reopen");
    ...
    absurder_sql::storage::import::import_database_from_bytes(&db_name, data).await
        .expect("Should import");
    
    // Step 6: Create NEW database instance to read imported data
    let mut reopened_db = Database::new(target_config).await.expect("Should create new instance");
