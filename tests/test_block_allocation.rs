...
async fn test_block_allocation_and_deallocation() {
    ...
    storage.write_block(block1, test_data.clone()).await
        .expect("Should write to allocated block");
    
    // Test reading from allocated block
    let read_data = storage.read_block(block1).await
...
async fn test_block_allocation_tracking() {
    ...
    assert_eq!(storage.get_allocated_count(), 0, "Should start with 0 allocated blocks");
    
    // Allocate some blocks
    let _block1 = storage.allocate_block().await.expect("Should allocate");
    assert_eq!(storage.get_allocated_count(), 1, "Should have 1 allocated block");
    
    let _block2 = storage.allocate_block().await.expect("Should allocate");
    assert_eq!(storage.get_allocated_count(), 2, "Should have 2 allocated blocks");
    
    // Deallocate one block
    storage.deallocate_block(_block1).await.expect("Should deallocate");
    assert_eq!(storage.get_allocated_count(), 1, "Should have 1 allocated block after deallocation");
}
...
async fn test_block_allocation_errors() {
    ...
    assert!(result.is_err(), "Should error when deallocating non-existent block");
    
    // Test double deallocation
    let block = storage.allocate_block().await.expect("Should allocate");
    storage.deallocate_block(block).await.expect("Should deallocate first time");
